  ...
  return (
    <section className="relative bg-[#030712] min-h-screen pt-32 pb-20">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-sm text-[#8B5CF6]">
            <Sparkles size={14} />
            Let's Connect
          </div>
        ...
        >
          {contactMethods.map((method, index) => (
            <motion.a
              ...
              className="group relative flex flex-col items-center rounded-xl border border-white/5 bg-white/5 p-6 text-center transition-all hover:border-[#7C3AED]/30 hover:bg-white/10"
            >
              <div className={`rounded-full ${method.bg} p-3`}>
                <method.icon className={`h-6 w-6 ${method.color}`} />
              </div>
              <h3 className="mt-3 font-semibold text-white">{method.label}</h3>
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-[1.4fr_1fr]">
          ...
          <motion.div
            ...
            className="rounded-xl border border-white/5 bg-white/5 p-8 backdrop-blur-sm"
          >
            ...
            {isSubmitted ? (
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="mt-1 w-full rounded-lg border border-white/10 bg-[#030712] px-4 py-2.5 text-white placeholder-gray-500 transition-colors focus:border-[#7C3AED] focus:outline-none focus:ring-1 focus:ring-[#7C3AED]"
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#7C3AED] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#8B5CF6] disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
          ...
          >
            <div className="rounded-xl border border-white/5 bg-white/5 p-8 backdrop-blur-sm">
              ...
              <p className="mt-1 text-sm text-gray-400">Reach me directly via:</p>
              ...
              <div className="mt-6 space-y-4">
                {directInfo.map((item, index) => (
                  <motion.a
                    ...
                    href={item.href}
                    whileHover={{ x: 4 }}
                    className="flex items-center gap-4 rounded-lg border border-white/5 p-4 transition-colors hover:border-[#7C3AED]/30 hover:bg-white/5"
                  >
              ...
              <div className="mt-6 rounded-lg bg-[#7C3AED]/5 p-4 border border-[#7C3AED]/10">
                <p className="text-sm text-gray-400">
                  ...
                  <span className="text-white"> Telegram</span> and
