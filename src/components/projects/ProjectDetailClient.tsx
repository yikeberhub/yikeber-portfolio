  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="mt-6 relative aspect-video overflow-hidden rounded-xl border border-white/5"
      >
        <Image
          src={project.image}
          alt={project.title}
          ...
          <div className="flex gap-3">
            {project.github && (
              <a
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 text-sm text-gray-300 transition-colors hover:border-[#7C3AED]/30 hover:text-white"
              >
                <GithubIcon/>
              </a>
            )}
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-lg border border-white/5 bg-white/5 px-4 py-2 text-sm text-gray-300"
            >
              {tag}
            </span>
          ))}
        ...
        >
          ...
          <div className="mt-3 grid gap-4 sm:grid-cols-2">
            {project.screenshots.map((screenshot, index) => (
              <div
                key={index}
                className="relative aspect-video overflow-hidden rounded-lg border border-white/5"
              >
                <Image
                  src={screenshot}
      ...
      <motion.div
        ...
        className="mt-12 rounded-xl border border-white/5 bg-white/5 p-8 text-center backdrop-blur-sm"
      >
        <Sparkles className="mx-auto h-8 w-8 text-[#7C3AED]" />
        <h3 className="mt-3 text-xl font-semibold text-white">Interested in this project?</h3>
